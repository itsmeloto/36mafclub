...
const InfoModal: React.FC<InfoModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-3 sm:p-4">
      <div className="bg-gray-900/95 border border-gray-700/50 rounded-xl sm:rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-gray-900/95 border-b border-gray-700/50 p-4 sm:p-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-blue-500/20 rounded-lg">
              <Info className="w-5 h-5 sm:w-6 sm:h-6 text-blue-400" />
            </div>
            <h2 className="text-lg sm:text-2xl font-bold text-white">Խաղի տեղեկություններ</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="sticky bottom-0 bg-gray-900/95 border-t border-gray-700/50 p-4 sm:p-6">
          <div className="flex justify-center">
            <button
              onClick={onClose}
              className="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg transition-colors text-sm sm:text-base"
            >
              Հասկացա!
            </button>
          </div>
